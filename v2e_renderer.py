    ...
    def render_events_to_frames(self, event_arr: np.ndarray,
        ...
        while not doneWithTheseEvents:
            ...
            if not doneWithTheseEvents or self.exposure_mode==ExposureMode.SOURCE:
                ...
                # Signed event strength:
                # currentFrame > 0 : ON
                # currentFrame < 0 : OFF
                signed = np.clip(
                    self.currentFrame / float(self.full_scale_count),
                    -1.0,
                    1.0
                )
                ...
                # OpenCV uses BGR
                # ON  -> RED
                color_img[on_mask, 0] = 127 * (1 - mag[on_mask])   # B
                color_img[on_mask, 1] = 127 * (1 - mag[on_mask])   # G
                color_img[on_mask, 2] = 127 + 128 * mag[on_mask]   # R

                # OFF -> BLUE
                color_img[off_mask, 0] = 127 + 128 * mag[off_mask] # B
                color_img[off_mask, 1] = 127 * (1 - mag[off_mask]) # G
                color_img[off_mask, 2] = 127 * (1 - mag[off_mask]) # R
