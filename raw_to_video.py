#!/usr/bin/env python3
"""
raw_to_video.py — Render a .raw recording (evs_recorder.cpp) as a watchable video.
...
does not exist here: evs_recorder.cpp writes a standard Prophesee .raw file
...
----------------------------------------------------------------------------
  ...
  2. Pure-python EVT3.0 fallback — decodes the whole file up front via
     inspect_evt3.decode_evt3() + raw_to_events.unwrap_evt3_time() (the SAME
...
from inspect_evt3 import decode_evt3
...
def _frames_from_sdk(path: Path, frame_period_us: int, width, height, max_frames):
    ...
    # get_size() returns (height, width) -- see raw_to_events.py's _decode_sdk()
    # comment for the citation; same swap risk here if unpacked the other way.
    sdk_h, sdk_w = it.get_size()
    w = width or sdk_w
    h = height or sdk_h
    n = 0
    for ev in it:
...
def main():
    ...
    if not args.force_fallback:
        try:
            # Same generator-laziness pitfall as raw_to_events.py's _decode_sdk():
            # _frames_from_sdk() is a generator function, so calling it here runs
            # none of its body (including the metavision_core import) until the
            # first next() in the write loop below. Probe the import eagerly so
            # a missing package is caught here, not as an uncaught crash mid-loop.
