#!/usr/bin/env python3
"""
raw_to_events.py -- Prophesee .raw (from evs_recorder.cpp) -> events.h5
...
evs_recorder.cpp is a different program, talking to the same physical
...
sensor (see probe_metavision.cpp and evs_recorder.cpp's header comment).
...
Two decode backends, tried in this order:
  ...
  2. Pure-python EVT3.0 fallback -- no SDK install needed. Reuses
     inspect_evt3.decode_evt3(), the SAME word-level decoder cevt_to_events.py
...
SIDECAR .meta.json
-------------------
evs_recorder.cpp writes <output>.meta.json next to the .raw with camera
...
calibrate_simulator.py's check_timestamp_precision() requires status to be in
{"device_buffer", "precise"} for Eq.30/timing calibration unless overridden
with --allow-degraded-t ("unknown" and "host_arrival" are both REJECTED, not
passed through -- see that function's docstring) -- files from this script
pass that gate; files from the dense Arena path normally do not, by design.
...
from inspect_evt3 import (  # noqa: E402  -- one decoder, not a fourth copy
    decode_evt3,
)
...
def _load_sidecar_meta(raw_path: Path):
    """Read <raw_path>.meta.json written by evs_recorder.cpp, if present.
...
def _decode_sdk(path: Path, max_events):
    ...
    # get_size() returns (height, width) -- confirmed against Prophesee's own
    # docs/examples ("height, width = mv_iterator.get_size()"). Unpacking this
    # as (w, h) would silently swap width/height into events.h5's attrs, and
    # downstream consumers indexing arrays as (H, W) (hot_pixel_heatmap.py's
    # `counts = np.zeros((H, W))`) would then IndexError the first time an x
    # coordinate exceeds the swapped-in "width".
    h, w = it.get_size()
    total = 0
    for ev in it:
        if ev.size == 0:
...
def unwrap_evt3_time(ts_raw) -> np.ndarray:
    """Unwrap EVT3.0's 24-bit on-wire timestamp (wraps every 2**24 us, ~16.8s)
    into one continuous, monotonic microsecond timeline. ts_raw: raw
    (time_high<<12 | time_low) values as returned by inspect_evt3.decode_evt3(),
    each in [0, 2**24). Whenever a value drops relative to the previous one, a
    wraparound happened. Shared by _decode_evt3_fallback() below and by
    raw_to_video.py's fallback backend — one definition, not two copies that
    can silently diverge (raw_to_video.py imports this function directly)."""
    ts_raw = np.asarray(ts_raw, dtype=np.int64)
    if ts_raw.size == 0:
        return ts_raw.astype(DT_T)
    wraps = np.cumsum(np.diff(ts_raw, prepend=ts_raw[0]) < 0)
    return ts_raw + wraps.astype(np.int64) * EVT3_WRAP_US


def _decode_evt3_fallback(path: Path, max_events):
    """Backend 2: pure-python, no SDK required. Reuses inspect_evt3's
    ...
    # on-wire word format carries. unwrap_evt3_time() folds the wraps back in
    # (extracted to a standalone function so raw_to_video.py's fallback
    # backend can import and reuse it instead of a second copy).
    ts = unwrap_evt3_time(ts_raw)
    n_wraps = int(np.sum(np.diff(ts_raw, prepend=ts_raw[0]) < 0)) if len(ts_raw) else 0
    if n_wraps:
...
def main():
    ...
    ap.add_argument("path", help="Path to the .raw file (from evs_recorder.cpp)")
    ...
    if not args.force_fallback:
        try:
            # _decode_sdk() is a generator function -- calling it here does NOT
            # run any of its body (including the `from metavision_core...`
            # import) until the first `next()`, which happens later in the
            # write loop below. Catching ImportError around the call itself
            # therefore never actually catches anything; probe the import
            # eagerly here instead, so a missing package is caught cleanly
            # before any h5py output file has been created.
            import metavision_core.event_io  # noqa: F401
        except ImportError:
    ...
    with h5py.File(out, "w") as f:
        ...
        # calibrate_simulator.py's check_timestamp_precision() requires status in
        # {"device_buffer", "precise"} for Eq.30/timing calibration ("unknown" is
        # REJECTED, not passed through -- see module docstring above and that
        # function's own docstring; this comment previously said the opposite).
        ...
        if meta:
            ...
            erc = meta.get("erc", {})
            run = meta.get("run", {})
            f.attrs["recorder"] = meta.get("recorder", "evs_recorder")
